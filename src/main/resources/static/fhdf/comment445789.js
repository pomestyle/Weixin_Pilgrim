...
checkAjaxDo(e),e&&(e.success=function(i){
dealWithSucceed(i,e);
},e.error=function(){
dealWithFailed(e);
}),ajax(e);
...
var i=isContainExceptLike(e,failedQueue),a=isContainAjax(e,failedQueue);
-1===a&&i>-1&&failedQueue.splice(i,1);
}
function isContainExceptLike(e,i){
var a=-1;
for(var r in i){
var u=i[r];
if(e.url||-1!=e.url.indexOf("&like=")||-1!=u.url.indexOf("&like=")){
if(!(e.url.indexOf("&like=")>-1&&u.url.indexOf("&like=")>-1))continue;
if(removeLikeParam(e.url)!==removeLikeParam(u.url))continue;
}else if(!u.url||u.url!==e.url)continue;
if(e.data&&u.data){
var t=e.data,n=u.data;
if(!isEqualExceptLike(t,n))continue;
}
a=r;
break;
}
...
function isContainAjax(e,i){
var a=-1;
for(var r in i){
var u=i[r];
if(e.url&&u.url&&e.url==u.url){
if(e.data&&u.data){
var t=e.data,n=u.data;
if(!isEqual(t,n))continue;
}
a=r;
break;
}
...
var i=e.indexOf("&like="),a=e.substring(0,i)+e.substring(i+7);
return a;
}
function isEqualExceptLike(e,i){
var a=Object.getOwnPropertyNames(e),r=Object.getOwnPropertyNames(i);
if(a.length!=r.length)return!1;
for(var u=0;u<a.length;u++){
var t=a[u];
if("like"!==t&&e[t]!==i[t])return!1;
}
return!0;
}
function isEqual(e,i){
var a=Object.getOwnPropertyNames(e),r=Object.getOwnPropertyNames(i);
if(a.length!=r.length)return!1;
for(var u=0;u<a.length;u++){
var t=a[u];
...
var i=isContainExceptLike(e,failedQueue);
if(-1===i){
if(e.failedTimes=1,failedQueue.length>=MAX_QUEUE_LEN)return;
failedQueue.push(e);
}else{
var a=isContainAjax(e,failedQueue);
if(a>-1){
if(failedQueue[i].failedTimes++,e.failedTimes=failedQueue[i].failedTimes,e.failedTimes>MAX_FAILED_TIMES)return void failedQueue.splice(a,1);
}else failedQueue.splice(a,1),e.failedTimes=1,failedQueue.push(e);
}
Retry_ajax(e);
}
...
!!S&&console.info("[图文评论] 开始请求评论数据:",c),W("[Appmsg comment] start get comment data, url:"+c),
Y({
url:c,
type:"get",
success:function(t){
...
if(G.log("tag3"),e.length<1)return w("留言不能为空");
if(G.log("tag4"),e.length>600)return w("字数不能多于600个");
...
!!S&&console.info("[图文评论] 评论ID:",N),-1!=navigator.userAgent.indexOf("MicroMessenger")||window.test_comment_data||(L&&(L.style.display="none"),
...
var $=e("appmsg/emotion/emotion.js"),G=e("appmsg/emotion/dom.js"),K=(new Image,0),V=100,et=!1,tt="",nt="我",ot=0,mt={
